...
export const Standard: StoryObj<{
  ...
  render: (args) => ({
    ...
    setup() {
      const getNow = () => NOW
      // Re-key on showAllEvents/weekEventLimit to force store re-creation —
      // both are read once at initialization and are not reactive.
      const key = computed(() => `${String(args.showAllEvents)}-${args.weekEventLimit}`)
      return { localizer, events: demoEvents, FOCUS, getNow, args, key }
    },
    template: `
...
export const ScrollToTime: StoryObj<{ scrollToHour: number }> = {
  ...
  render: (args) => ({
    ...
    setup() {
      const getNow = () => NOW
      const key = computed(() => `scroll-${args.scrollToHour}`)
      return { localizer, events: demoEvents, FOCUS, getNow, args, key }
    },
    template: `
      <div style="block-size: 100dvh; inline-size: 100%">
...
export const TimeWindow: StoryObj<{
  ...
  render: (args) => ({
    ...
    setup() {
      const getNow = () => NOW
      const key = computed(() => `window-${args.minHour}-${args.maxHour}`)
      return { localizer, events: demoEvents, FOCUS, getNow, args, key }
    },
    template: `
      <div style="block-size: 100dvh; inline-size: 100%">
