 ...
 * active view is not the month (so `<MonthView>` can bow out cleanly).
 */
export function useMonthWeeks<TEvent>(): ComputedRef<MonthGrid<TEvent> | null> {
  const { store } = useCalendarContext<TEvent>()
  const viewModelRef = useSignalRef(store.viewModel)
  const focusRef = useSignalRef(store.date)
