...
describe.each(LOCALIZER_CASES)('createCalendarStore [$name]', ({ create }) => {
  ...
  describe('createCalendarStore', () => {
    ...
    it('eventHandlers fire only the configured callbacks (no noop fabrication)', () => {
      ...
      store.eventHandlers.click(ev, domEvent)
      expect(onEventClick).toHaveBeenCalledWith(ev, domEvent)
      expect(store.selected.value).toBeNull()
      store.eventHandlers.doubleClick(ev, domEvent)
      expect(onEventDoubleClick).toHaveBeenCalledWith(ev, domEvent)
    ...
    it('eventHandlers report no presence and never throw when nothing is configured', () => {
      ...
      expect(() => {
        store.eventHandlers.click(ev, {} as MouseEvent)
        store.eventHandlers.doubleClick(ev, {} as MouseEvent)
