...
/**
 * Props accepted by `injectCalendarStore` and `CalendarProviderComponent`.
 * Mirrors the React and Vue adapters' `CalendarProps` for API parity.
 *
...
export interface CalendarProps<TEvent = unknown, TResource = unknown>
  ...
  /**
   * Which view keys to show in the toolbar and allow navigation to. Defaults
   * to all built-in views plus any custom keys in `viewDefinitions`.
   */
