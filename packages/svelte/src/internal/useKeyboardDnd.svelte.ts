...
/**
 * Svelte 5 port of the Vue `useKeyboardDnd` composable.
 *
 ...
 * Enter drops, Escape cancels. Returns a `getAnnouncement()` getter to drive a
...
export function useKeyboardDnd({ mode }: { mode: MoveMode }): KeyboardDnd {
  ...
  const onKeydownCapture = (e: KeyboardEvent): void => {
    ...
    if (grab == null) {
      ...
      e.preventDefault()
      e.stopPropagation()
      announcement = describe(id, pickupHint)
      return
    }
