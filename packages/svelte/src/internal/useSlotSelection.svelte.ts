...
/**
 * Svelte 5 port of the Vue `useSlotSelection` composable.
 *
 * Pointer-driven slot selection for one surface (`'time'` or `'day'`).
 * Returns an `onPointerdown` handler to attach to the selection surface.
 */
  ...
  return (e: PointerEvent): void => {
    ...
    const onUp = (): void => {
      ...
      if (
        ...
        tap = null
        store.selection.doubleClick({ slot, date, mode, slotCount: getSlotCount?.(), resourceId: anchor.resourceId })
        return
      }
      if (prev != null) clearTimeout(prev.timer)
      const timer = setTimeout(() => {
        tap = null
        store.selection.click({ slot, date, mode, slotCount: getSlotCount?.(), resourceId: anchor.resourceId })
      }, DOUBLE_CLICK_MS)
